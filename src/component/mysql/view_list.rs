use crate::{
    ...
    dialog::{ConfirmDialog, Kind as ConfirmKind},
    event::{config::*, Key},
    model::mysql::{get_mysql_views, Connections, View},
...
impl ViewListComponent {
    ...
    pub async fn set_data(&mut self, conn_id: &Uuid, db_name: &str) -> Result<()> {
        ...
        .await?;
        self.views = get_mysql_views(&pool, db_name).await?;
        Ok(())
    }
        ...
        let table = Table::new(
            self.views
                ...
                .map(|view| {
                    RowUI::new(vec![
                        view.name.clone(),
                        view.check_option.clone().unwrap_or_default(),
                        view.definer.clone().unwrap_or_default(),
                        view.sql_security.clone().unwrap_or_default(),
                    ])
                })
                .collect::<Vec<RowUI>>(),
        )
        ...
        match *key {
            ...
            CONFIRM_KEY => {
                if let Some(index) = self.state.selected() {
                    let select_view = &self.views[index];
                    return Ok(ComponentResult::Goto(Goto::DataListMySQL {
                        conn_id: self.conn_id.unwrap(),
                        db_name: self.db_name.clone().unwrap(),
                        table_name: select_view.name.clone(),
                    }));
                }
            }
            NEW_KEY => {
                return Ok(ComponentResult::Goto(Goto::ViewDetailMySQL {
                    conn_id: self.conn_id.unwrap(),
                    db_name: self.db_name.clone().unwrap(),
                    view_name: None,
                }));
    ...
    async fn refresh(&mut self) -> Result<()> {
        ...
        .await?;
        self.views = get_mysql_views(&pool, self.db_name.as_ref().unwrap()).await?;
